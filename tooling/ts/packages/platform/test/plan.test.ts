...
describe("planPlatform — manifest → shadcn adds + entry + provision.config", () => {
  ...
  test("the generated provision.config is a defineProvisionApp: merges each fragment + wires brokers/store/sink", () => {
    // a full ProvisionApp (what `suluk-provision`/`bun run provision` load), NOT a bare config — else runCli crashes on app.store.
    expect(plan.provisionConfig).toContain("export default defineProvisionApp({");
    ...
    expect(plan.provisionConfig).toContain("config: defineProvision({ instances: mergeProvision([authProvision, creditsProvision, keysProvision, billingProvision, logsProvision]) })");
    expect(plan.provisionConfig).toContain('"cloudflare-d1": cloudflareD1(cf)'); // the broker registry
    expect(plan.provisionConfig).toContain("store: fileStore()");
    expect(plan.provisionConfig).toContain("sink: envSink()");
  });
...
describe("package.json generation — the manifest is the only surface", () => {
  ...
  test("mergePackageJson PRUNES a stale `predev` gate (it would block mock-until-keyed `bun dev`)", () => {
    const baseline = buildPackageJson("myapp", ["auth", "credits"]);
    // an app generated by an OLDER generator that wired env-check as `predev` — the stale gate must NOT survive a regen.
    const existing = JSON.stringify({ name: "myapp", scripts: { predev: "bun run scripts/env-check.ts", "my-lint": "eslint ." } });
    const merged = JSON.parse(mergePackageJson(baseline, existing));
    expect(merged.scripts.predev).toBeUndefined(); // pruned → `bun dev` is no longer gated
    expect(merged.scripts["my-lint"]).toBe("eslint ."); // a genuine app script still survives
    expect(merged.scripts.dev).toBe("bun run --hot src/dev.ts"); // dev itself intact
  });

  test("the predev prune is SURGICAL — a legitimately app-authored predev (not the env-check gate) survives", () => {
    const baseline = buildPackageJson("myapp", ["auth"]);
    const existing = JSON.stringify({ name: "myapp", scripts: { predev: "bun run codegen" } }); // an app's OWN pre-dev hook
    const merged = JSON.parse(mergePackageJson(baseline, existing));
    expect(merged.scripts.predev).toBe("bun run codegen"); // value ≠ the stale env-check gate → NOT pruned
  });
