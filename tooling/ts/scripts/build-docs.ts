...
const PLATFORM_UML = "https://cdn.jsdelivr.net/gh/MahmoodKhalil57/suluk@main/tooling/ts/docs-pages/platform-registry-uml.svg";
...
export async function buildDocs(): Promise<DocPackage[]> {
  ...
  for (const p of pkgs) {
    ...
    // @suluk/platform is the assembler: give it a visual (d3) projection of the whole registry, each module
    // stereotyped with the `definePlatform` service that composes it (platform-registry-uml.svg, autogenerated).
    if (p.name === "@suluk/platform") {
      const projDoc = join(tmp, "platform-registry.md");
      writeFileSync(
        projDoc,
        `---\ntitle: Registry it assembles\ngroup: Documents\n---\n\n# The registry \`@suluk/platform\` assembles\n\n\`definePlatform\` composes a backend from a set of **services**, each realized by an own-the-code [shadcn-registry](${UMBRELLA_URL}documents/Registry.html) module. This is that projection — every registry module below, tagged with the platform service (\`«…Service»\`) that composes it, laid out by how the modules build on each other (\`app\` is the foundation). Rendered with [d3](https://d3js.org).\n\n![The registry @suluk/platform assembles — services ⇒ modules](${PLATFORM_UML})\n`,
      );
      projectDocuments.push(projDoc);
    }
    let readme = "none";
