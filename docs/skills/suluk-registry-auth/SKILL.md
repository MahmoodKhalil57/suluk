...
`/api/*` FIRST, then the `/api/auth/*` handler), `getCurrentUser` (Resolve the session user from the request headers), `currentUserLayer` (A `CurrentUser` layer for one request — provide it alongside your feature service layers), `CurrentUser`, `AuthEnv`, `AppCtx`, `SessionUser`, `AppVars` (Request-scoped identity, set by identity (session) or apiKeyAuth (an `x-api-key`)), `apiKeyAuth` (Programmatic auth via an `x-api-key` header (the api-key plugin) — when there is NO session, verify the key and stash its
