...
[Suluk](../../../packages.md) / [Registry](../../Registry.md) / [Services](../Services.md) / email

# Email — swappable provider binding + auth lifecycle

An Effect-TS `Email` service over [`@suluk/email`](https://github.com/MahmoodKhalil57/suluk/tree/main/registry/tooling/ts/packages/email) — a stateless binding that reads the provider from env (console in dev, Resend in prod) and hands you `send` + `verify` + `reset` to wire straight into Better Auth.
...
> ecosystem, distributed as [own-the-code shadcn modules](https://github.com/MahmoodKhalil57/suluk/blob/main/registry/services/README.md).
...
infra. Want an audit trail of sends? Compose the [`logs`](https://github.com/MahmoodKhalil57/suluk/tree/main/registry/services/logs) module.
...
- **registry** (`registryDependencies`): [`app`](https://github.com/MahmoodKhalil57/suluk/tree/main/registry/services/app) — the base Hono app + the shared Effect runtime seam
