...
[Suluk](../../packages.md) / [Registry](../Registry.md) / email

# Email — swappable provider binding + auth lifecycle

An Effect-TS `Email` service over [`@suluk/email`](https://github.com/MahmoodKhalil57/suluk/tree/main/tooling/ts/packages/email) — a stateless binding that reads the provider from env (console in dev, Resend in prod) and hands you `send` + `verify` + `reset` to wire straight into Better Auth.
...
infra. Want an audit trail of sends? Compose the [`logs`](https://github.com/MahmoodKhalil57/suluk/tree/main/registry/logs) module.
...
- **registry** (`registryDependencies`): [`app`](https://github.com/MahmoodKhalil57/suluk/tree/main/registry/app) — the base Hono app + the shared Effect runtime seam
