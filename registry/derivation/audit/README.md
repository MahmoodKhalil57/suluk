...
- [`@suluk/cockpit`](../../tooling/ts/packages/cockpit) — `conformanceGates` + `shipSummary` (the lifecycle / ship-readiness gates)
- [`@suluk/harden`](../../tooling/ts/packages/harden) — `auditDocument` (security grade), `auditReadiness` (readiness grade), `combineGrades` (the two letters collapsed to worst + average), and the `Grade` type
- [`@suluk/core`](../../tooling/ts/packages/core) — the `OpenAPIv4Document` type the script loads and passes to every audit
...
This is the HYBRID pattern (ADR [C050](../../doc/architecture/decisions/C050-registry-distributed-framework.md)/[C052](../../doc/architecture/decisions/C052-npm-vs-registry-boundary.md)): you **own** the seam, and **npm** the grading logic.
