...
**Stateless — no schema, no provision fragment** ([C052](../../doc/architecture/decisions/C052-npm-vs-registry-boundary.md)). The contract is pure data + derivations.
...
- **registry (`registryDependencies`):** [`app`](../app) — the base Hono app the mount attaches to.

Downstream, [`reference`](../reference) and [`mcp`](../mcp) list `contract` as a `registryDependency`: both
