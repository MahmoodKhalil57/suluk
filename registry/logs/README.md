...
`registryDependencies` pull in [`app`](../app) (the base Hono app + the Effect `Db` service) automatically.
...
- **registry** (`registryDependencies`): [`app`](../app) — supplies the `Db` / `DbLive` Effect service and
  the `Bindings` type the routes run over.

## Own the wiring, npm the logic

This module is the exception to the [hybrid rule](../README.md#the-hybrid-rule): **there is no `@suluk`
