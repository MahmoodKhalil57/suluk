...
A principal-aware rate-limit middleware over [`@suluk/hono`](../../tooling/ts/packages/hono)'s
...
- **`src/services/rate-limit.ts`** — the owned middleware wiring over `@suluk/hono`'s `enforceRateLimit`:
  - **`principalKeyOf(c, facet)`** — the owned, principal-aware keyer. When a facet keys by `"principal"`
    ...
    Better Auth / [`auth`](../auth) convention), so a signed-in caller gets their own bucket regardless of
...
- **registry** (`registryDependencies`): [`app`](../app) — the base Hono app + the shared `Bindings` type
