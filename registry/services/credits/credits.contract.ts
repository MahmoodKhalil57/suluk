...
export const creditsOps = [
  { method: "get", path: "/api/credits", name: "getCredits", summary: "The caller's credit balance.", tags: ["Credits"], scopes: ["credits:read"], responses: [{ status: 200, description: "The current credit balance.", schema: Balance }] },
  { method: "get", path: "/api/credits/balance/:userId", name: "getUserCredits", summary: "A specific user's credit balance (self/admin).", tags: ["Credits"], scopes: ["credits:read"], responses: [{ status: 200, description: "The user's credit balance.", schema: Balance }] },
  { method: "get", path: "/api/credits/transactions", name: "listTransactions", summary: "The caller's recent credit ledger (grants + usage debits), newest first.", tags: ["Credits"], scopes: ["credits:read"], responses: [{ status: 200, description: "The credit transaction ledger.", schema: z.object({ transactions: z.array(Transaction) }) }] },
  {
