...
import { z } from "zod";
import { effectRoute, ExternalServiceError } from "@suluk/effect";
...
export const sendEmailRoute = effectRoute({
...
// provider-reject path → a TYPED 502 ExternalServiceError.
export const verifyEmailRoute = effectRoute({
...
export function emailRoutes() {
  ...
  r.post("/send", sendEmailRoute.handler);
  r.post("/verify", verifyEmailRoute.handler);
